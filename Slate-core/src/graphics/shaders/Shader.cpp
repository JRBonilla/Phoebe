...
namespace sl { namespace graphics {
	...
	GLuint Shader::Load() {
		...
		if (Compile(vertex, m_VertexPath) == FAILURE) {
			SL_LOG("Failed to Compile vertex shader!");
			return FAILURE;
		}

		if (Compile(fragment, m_FragmentPath) == FAILURE) {
			SL_LOG("Failed to Compile fragment shader!");
			return FAILURE;
		}

		GLuint program = glCreateProgram();
		if (Link(program, vertex, fragment) == FAILURE) {
			SL_LOG("Failed to create program!");
			return FAILURE;
		}
	...
	GLuint Shader::Compile(GLuint& id, const char* path) {
		...
		if (success == GL_FALSE) {
			...
			SL_LOG("Failed to Compile shader: " << path);
	...
	GLuint Shader::Link(GLuint& program, GLuint& vertex, GLuint& fragment) {
		...
		if (success == GL_FALSE) {
			...
			SL_LOG("Failed to Link shaders to program!");
			std::cout << &error[0] << std::endl;
